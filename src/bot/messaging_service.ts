...
export default class MessagingService {
    ...
    private async _internalSendMessage(
        ...
        await this.client!.sendPresenceUpdate("composing", recipient);
        ...
        try {
            ...
            if (sentMessage?.key.remoteJid && sentMessage.key.id)
                this.client!.sendReadReceipt(
                    sentMessage?.key.remoteJid,
                    sentMessage?.key.participant ?? sentMessage?.key.remoteJid,
                    [sentMessage?.key.id],
                );
                
            wait(1300).then(() => {
                whatsappBot.client!.sendPresenceUpdate("available", recipient);
            });
        }
    }
